...
def web_search_fallback(query: str) -> str:
    ...
    try:
        ...
        structured_prompt = PromptTemplate(
            template="""
            You are a Artificial Neural Networks (ANN) expert assistant. 
            A user asked: "{query}"
            ...
            Keep it concise and relevant to Artificial Neural Network concepts.
            """,
...
def load_and_create_vectordb(pdf_path='pdfs/ann_tb.pdf', 
                        vectordb_dir='vectordb/ann_faiss',
                        cache_dir='cache',
                        test_mode=False):
    """
    ...
    try:
        ...
        st.info("🔮 Creating vector embeddings... This may take a few minutes...")
        vectordb = FAISS.from_documents(documents, embeddings)
        
        # Save to disk
        st.info(f"💾 Saving vector database to: {vectordb_dir}")
...
def generate_chat_pdf(messages):
    ...
    # Add title
    title = Paragraph("ANN Chatbot Conversation", title_style)
...
def generate_quiz_from_history(chat_history):
    ...
    quiz_prompt = f"""
    You are a teacher. Based strictly on the following conversation history about Artificial Neural Network(ANN), generate 5 multiple-choice questions (MCQs) to test the user's understanding of the topics discussed.
...
def extract_questions_with_numbers(text):
    ...
    6. If a question has sub-parts (a, b), treat them as individual questions and then remove the (a, b, c) question name.
       Example: "1a :: Define AI" or "1 :: Define ANN".
...
def process_pyq_pdfs(folder_path=None, force_reprocess=False):
    ...
    if folder_path is None:
        ...
        folder_path = os.path.join(base_dir, 'pyq_pdfs', 'ann')
        
    output_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "PYQs/pyqs_master_ann.json")
...
def main():
    ...
    with st.sidebar:
        ...
        # if st.button("📕 ANN", use_container_width=True):
        ...
        if st.button("Generate Quiz from Chat", use_container_width=True):
            if len(st.session_state.ann_chat_history) < 2:
                ...
                with st.spinner("👩‍🏫 Analyzing conversation and crafting questions..."):
                    quiz = generate_quiz_from_history(st.session_state.ann_chat_history[-10:])
                    if quiz:
                        st.session_state.ann_quiz_data = quiz
        ...
        if st.button("🧠 Analyze PYQ Papers", use_container_width=True):
            with st.spinner("Reading PDFs, extracting questions, and checking duplicates..."):
                # Run the processor
                result = process_pyq_pdfs('pyq_pdfs/ann')
                ...
                if result == "FOLDER_MISSING":
                    ...
                    st.session_state.ann_show_pyq_results = True 
                    st.rerun()
        ...
        if st.button("📥 Download Conversation as PDF", use_container_width=True):
            if "ann_messages" in st.session_state and st.session_state.ann_messages:
                try:
                    pdf_data = generate_chat_pdf(st.session_state.ann_messages)
                    
                    # Create download button
                    st.download_button(
                        label="💾 Click to Download PDF",
                        data=pdf_data,
                        file_name=f"ann_chat_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
        ...
        if st.button("🗑️ Clear Chat History", use_container_width=True):
            ...
            st.session_state.ann_chat_history = []
            st.rerun()
        ...
        #     vectordb_path = 'vectordb/ann_faiss'
        #     try:
        ...
        #             st.session_state.ann_vectordb = load_and_create_vectordb(test_mode=test_mode)
                    
        #         if st.session_state.ann_vectordb:
        ...
        # Status indicator
        st.markdown("### 📊 Status")
        if "ann_vectordb" in st.session_state and st.session_state.ann_vectordb:
    ...
    if "ann_chat_history" not in st.session_state:
        st.session_state.ann_chat_history = []

    # for quiz questions
    if "ann_quiz_data" not in st.session_state:
        st.session_state.ann_quiz_data = None

    # for pyq visibility
    if "ann_show_pyq_results" not in st.session_state:
        st.session_state.ann_show_pyq_results = False
    
    if "ann_vectordb" not in st.session_state:
        st.info("🚀 Initializing ANN knowledge base...")
        st.info("💡 Tip: First time setup may take 5-10 minutes. Results will be cached for future use.")
        
        try:
            with st.spinner("Loading ANN knowledge base... Please wait..."):
                st.session_state.ann_vectordb = load_and_create_vectordb()
...
# --- PYQ DASHBOARD SECTION ---
    ...
    json_path = os.path.join(project_root, "PYQs/pyqs_master_ann.json")

    # COMBINED CHECK: File must exist AND the flag must be True
    if st.session_state.ann_show_pyq_results and os.path.exists(json_path):
    ...
    # Get user input
    user_input = st.chat_input("Ask anything about ANN...")

    # Process user input
    if user_input:
        # Add user message to session state
        st.session_state.ann_messages.append({
            "role": "user",
            "content": user_input
        })

        current_chat_history = st.session_state.ann_chat_history.copy()
        ...
        if is_casual and len(user_input.split()) <= 3:
            prompt = ChatPromptTemplate.from_messages([
                ("system", "You are a friendly ANN expert assistant. Respond warmly to greetings."),
        ...
        # Handle regular questions with RAG
            ...
            retriever = st.session_state.ann_vectordb.as_retriever(
                search_type="similarity",
                search_kwargs={"k": 4}
            )

            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are an expert Artificial Neural Network (ANN) assistant who has mastered all the concepts of Artificial Neural Network.
Answer the question using the context extracted from the ANN textbook and the previous conversation.
...
{context}"""),
        ...
        # Update chat history
        st.session_state.ann_chat_history.append(HumanMessage(content=user_input))
        st.session_state.ann_chat_history.append(AIMessage(content=answer))
        st.session_state.ann_messages.append({"role": "assistant", "content": answer})
    ...
    with chat_container:
        for message in st.session_state.ann_messages:  
            if message["role"] == "user":
                st.markdown(f"""
    ...
    if st.session_state.ann_quiz_data:
        st.markdown("---")
        with st.expander("📝 Take the Quiz", expanded=True):
            ...
            with st.form("quiz_form"):
                ...
                for i, q in enumerate(st.session_state.ann_quiz_data):
                ...
                if submitted:
                    ...
                    percentage = (correct_count / len(st.session_state.ann_quiz_data)) * 100
                    st.metric(label="Final Score", value=f"{percentage}%", delta=f"{correct_count}/{len(st.session_state.ann_quiz_data)} Correct")
